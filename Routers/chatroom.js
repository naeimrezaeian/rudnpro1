...
const { chatroomSchema, validate } = require('../Middleware/validator.js');
...
router.get('/:id',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT]), async function (req, res)  {
    const Id=req.params.id
        await Database.ChatRoom.findOne({
       where: {
           Id: Id,
           status: 1
       }
...
router.get('/Filter/:roomaccess/:offset/:limit',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT]), async function (req, res)  {
 
   const RoomAccess = req.params.roomaccess
   const offset = parseInt(req.params.offset) || 0
   const limit = parseInt(req.params.limit) || 0    

   let where = {status: 1};
   let limits = {};     
   if (RoomAccess !=0){  where.RoomAccess=RoomAccess }  
   if (limit != 0 ){      limits={ offset: offset,limit: limit }      }

       await Database.ChatRoom.findAndCountAll({
      where: where,
      ...limits,
    ...
    router.delete('/',authChek,authRole([Config.ROLE.ADMIN]), async function(req, res)  {
        ...
        if (items !=null){                     
            itemsDelete = await Database.ChatRoom.destroy({ 
                where: { id: [items] }
            }).then( (response) =>{
                if (response > 0 ){
    ...
    router.post('/',authChek,authRole([Config.ROLE.ADMIN]),validate(chatroomSchema),FindDuplicate(Database.ChatRoom,["roomName","roomAccess"])
,async function (req,res) {  
   
    
    Database.ChatRoom.create({                  
        RoomName: req.body.roomName,    
        RoomAccess: req.body.roomAccess,        
...
router.put('/:id',authChek,authRole([Config.ROLE.ADMIN]),validate(chatroomSchema),FindDuplicate(Database.ChatRoom,["roomName","roomAccess"]),
async function  (req, res) {   

   
    const dataId = parseInt(req.params.id)  
   
     await Database.ChatRoom.update(
        {
            RoomName: req.body.roomName,    
            RoomAccess: req.body.roomAccess,        
            Status : req.body.status
