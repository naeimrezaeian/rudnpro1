...
const { examratingSchema, validate } = require('../Middleware/validator.js');
...
router.get('/:id',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT,Config.ROLE.TEACHER]), async function (req, res)  {
    const Id=req.params.id
        await Database.ExamRatting.findOne({
       where: {
           Id: Id,
           status: 1
       }
...
router.get('/Filter/:examid/:studentid/:offset/:limit',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT,Config.ROLE.TEACHER]), async function (req, res)  {
   ...
   if (limit != 0 ){      limits={ offset: offset,limit: limit }      }
       ...
       await Database.ExamRatting.findAndCountAll({
      where: where,
      ...limits,
    ...
    router.delete('/',authChek,authRole([Config.ROLE.ADMIN]), async function(req, res)  {
        ...
        if (items !=null){                     
            itemsDelete = await Database.ExamRatting.destroy({ 
                where: { id: [items] }
            }).then( (response) =>{
                if (response > 0 ){
    ...
    router.post('/',authChek,authRole([Config.ROLE.ADMIN]),validate(examratingSchema),
    FindDuplicate(Database.ExamRatting,["examId","studentId"])
,async function (req,res) {  

    Database.ExamRatting.create({                  
...
router.put('/:id',authChek,authRole([Config.ROLE.ADMIN]),validate(examratingSchema),
FindDuplicate(Database.ExamRatting,["examId","studentId"]),
async function  (req, res) {   
    ...
    const dataId = parseInt(req.params.id)  
     ...
     await Database.ExamRatting.update(
        {
        ExamId: req.body.examId,    
        StudentId: req.body.studentId, 
        Ratting: req.body.ratting,
