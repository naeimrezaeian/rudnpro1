...
const { departmentSchema, validate } = require('../Middleware/validator.js');
...
router.get('/:id',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT,Config.ROLE.TEACHER]), async function (req, res)  {
   const Id=req.params.id
       await Database.Department.findOne({
      where: {
          Id: Id,
          status: 1
      }
 ...
 router.get('/Filter/:facultyId/:offset/:limit',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT,Config.ROLE.TEACHER]), async function(req, res)  {
    ...
    if (facultyId !=0){  where.facultyId=facultyId } 
    await Database.Department.findAndCountAll({
        where: {  ...where     },
        ...limits,
        attributes:['id','title','description','facultyId',[Database.Sequelize.col('faculty.title'), 'facultyTitle']],
        include:{ model:Database.Faculty,attributes: []},
    }).then(result => { 
        metadata = {offset:offset,limit:limit,total:result.count}
    res.status(200).json({  data :{items:result.rows,metadata:metadata} }) 
...
router.delete('/',authChek,authRole([Config.ROLE.ADMIN]), async function(req, res)  {
        ...
        if (items !=null){                     
            itemsDelete = await Database.Department.destroy({ 
                where: { id: [items] }
            }).then( (response) =>{
                if (response > 0 ){
 ...
 router.post('/',authChek,authRole([Config.ROLE.ADMIN]),validate(departmentSchema),FindDuplicate(Database.Department,["title"])
...
router.put('/:id',authChek,authRole([Config.ROLE.ADMIN]),validate(departmentSchema),FindDuplicate(Database.Department,["title"]),
async function  (req, res) {   
