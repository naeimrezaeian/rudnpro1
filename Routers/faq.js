...
const { faqSchema, validate } = require('../Middleware/validator.js');
...
router.get('/Filter/:groupid/:offset/:limit',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT,Config.ROLE.TEACHER]), async function (req, res)  {
   ...
   if (limit != 0 ){      limits={ offset: offset,limit: limit }      }
       ...
       await Database.Faq.findAndCountAll({
      where: where,
      ...limits,
    ...
    router.delete('/',authChek,authRole([Config.ROLE.ADMIN]), async function(req, res)  {
        ...
        if (items !=null){                     
            itemsDelete = await Database.Faq.destroy({ 
                where: { id: [items] }
            }).then( (response) =>{
                if (response > 0 ){
    ...
    router.post('/',authChek,authRole([Config.ROLE.ADMIN]),validate(faqSchema),FindDuplicate(Database.Faq,["groupId","question"])
...
router.put('/:id',authChek,authRole([Config.ROLE.ADMIN]),validate(faqSchema),FindDuplicate(Database.Faq,["groupId","question"]),
async function  (req, res) {   
