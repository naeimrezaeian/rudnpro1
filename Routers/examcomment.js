...
const { examcommentSchema, validate } = require('../Middleware/validator.js');
const {FindDuplicate }= require('../Middleware/Duplicate')
const router = express.Router()

router.get('/:id',authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT,Config.ROLE.TEACHER]), async function (req, res)  {
    const Id=req.params.id
        await Database.ExamComment.findOne({
       where: {
           Id: Id,
           status: 1
       }
...
router.get('/Filter/:examid/:studentid/:questionid/:offset/:limit',authChek,authRole([Config.ROLE.ADMIN,Config.ROLE.STUDENT,Config.ROLE.TEACHER]), async function (req, res)  {
   ...
   if (limit != 0 ){      limits={ offset: offset,limit: limit }      }
       ...
       await Database.ExamComment.findAndCountAll({
      where: where,
      ...limits,
    ...
    router.delete('/',authChek,authRole([Config.ROLE.ADMIN]), async function(req, res)  {
        ...
        if (items !=null){                     
            itemsDelete = await Database.ExamComment.destroy({ 
                where: { id: [items] }
            }).then( (response) =>{
                if (response > 0 ){
    ...
    router.post('/',authChek,authRole([Config.ROLE.ADMIN]),validate(examcommentSchema)    
,async function (req,res) {  
  
    Database.ExamComment.create({                  
...
router.put('/:id',authChek,authRole([Config.ROLE.ADMIN]),validate(examcommentSchema),
async function  (req, res) {   

   
    const dataId = parseInt(req.params.id)  
   
     await Database.ExamComment.update(
        {
            ExamId: req.body.examId,    
            StudentId: req.body.studentId, 
            QuestionId: req.body.questionId,
